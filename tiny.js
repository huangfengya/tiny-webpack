...
/***
 * 从入口开始分析所有依赖项，形成依赖图,采用广度遍历，一边循环，一边网循环目标中push。
 * 输入 str（文件路径）
...
function createGraph(entry) {
  ...
  //既然要广度遍历肯定要有一个队列，第一个元素肯定是 从 "./index.js" 返回的对象
  const queue = [mainAsset];
  // 遍历存放文件对象的数组
  for (const asset of queue) {
    //获取文件路径  './index.js' 返回 './es6ToEs5'
...
const graph = createGraph("./index.js");//[文件对象，文件对象，文件对象...]
